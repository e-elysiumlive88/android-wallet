...
package de.schildbach.wallet.elysium.util;
...
import com.google.elysium.core.DumpedPrivateKey;
import com.google.elysium.core.ECKey;
import com.google.elysium.core.ScriptException;
import com.google.elysium.core.Sha256Hash;
import com.google.elysium.core.Transaction;
import com.google.elysium.core.TransactionInput;
import com.google.elysium.core.TransactionOutput;
...
public class WalletUtils
{
	...
	public static Bitmap getQRCodeBitmap(final String url, final int size)
	{
		try
		{
			...
			String els_url = url.replace("elysium:","elysium:");

			final Hashtable<EncodeHintType, Object> hints = new Hashtable<EncodeHintType, Object>();
			hints.put(EncodeHintType.MARGIN, 0);
			hints.put(EncodeHintType.ERROR_CORRECTION, ErrorCorrectionLevel.H);
			final BitMatrix result = QR_CODE_WRITER.encode(els_url, BarcodeFormat.QR_CODE, size, size, hints);
	...
	public static BigInteger localValue(final BigInteger elsmValue, final BigInteger rate)
	{
		return elsmValue.multiply(rate).divide(Utils.COIN);
	}

	public static BigInteger elsmValue(final BigInteger localValue, final BigInteger rate)
	{
	...
	public static void writeKeys(final Writer out, final List<ECKey> keys) throws IOException
	{
		...
		out.write("# KEEP YOUR PRIVATE KEYS SAFE! Anyone who can read this can spend your Elysium.\n");
    ...
    public static ElysiumURI parseAddressString(String addressString) {
        if (addressString == null) return null;
        Log.d("Elysium", "Parsing: " + addressString);
        try {
            final ElysiumURI uri = new ElysiumURI(Constants.NETWORK_PARAMETERS, addressString);
            Log.d("Elysium", "URI: " + uri.getAddress().toString() + " " + uri.getLabel() + " " + uri.getAmount());
            return uri;
        }
        catch (final ElysiumURIParseException x)
        {
            // Try prepending elysium:
            try {
                final ElysiumURI uri = new ElysiumURI(Constants.NETWORK_PARAMETERS, "elysium:" + addressString);
                Log.d("Elysium", "URI: " + uri.getAddress().toString() + " " + uri.getLabel() + " " + uri.getAmount());
                return uri;
            }
            catch (final ElysiumURIParseException y)
            {
                return null;
            }
        }
