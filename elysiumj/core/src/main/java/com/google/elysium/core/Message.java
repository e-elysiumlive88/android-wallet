...
/**
 * <p>A Message is a data structure that can be serialized/deserialized using both the Elysium proprietary serialization
...
public abstract class Message implements Serializable {
     ...
     * @param msg Elysium protocol formatted byte array containing message content.
     * @param offset The location of the first msg byte within the array.
     * @param protocolVersion Elysium protocol version.
    ...
    private void selfCheck(byte[] msg, int offset) {
        if (!(this instanceof VersionMessage)) {
            ...
            byte[] reserialized = elysiumSerialize();
    ...
    // These methods handle the serialization/deserialization using the custom Elysium protocol.
    ...
    /**
     * Should only used by ElysiumSerializer for cached checksum
     *
     * @return the checksum
     */
    byte[] getChecksum() {
    ...
    /**
     * Should only used by ElysiumSerializer for caching checksum
     *
    ...
    /**
     * Returns a copy of the array returned by {@link Message#unsafeElysiumSerialize()}, which is safe to mutate.
     ...
     */
    public byte[] elysiumSerialize() {
        byte[] bytes = unsafeElysiumSerialize();
    ...
    /**
     * Serialize this message to a byte array that conforms to the elysium wire protocol.
     * <br/>
     ...
     */
    public byte[] unsafeElysiumSerialize() {
        ...
        try {
            elysiumSerializeToStream(stream);
        } catch (IOException e) {
        ...
        if (parseRetain) {
            ...
            // which means another call to elysiumSerialize is coming.  If we didn't recache then internal
    ...
    /**
     * Serialize this message to the provided OutputStream using the elysium wire format.
     *
     * @param stream
     * @throws IOException
     */
    final public void elysiumSerialize(OutputStream stream) throws IOException {
    ...
    /**
     * Serializes this message to the provided stream. If you just want the raw bytes use elysiumSerialize().
     */
    void elysiumSerializeToStream(OutputStream stream) throws IOException {
        log.debug("Warning: {} class has not implemented elysiumSerializeToStream method.  Generating message with no payload", getClass());
    }

    /**
     * This method is a NOP for all classes except Block and Transaction.  It is only declared in Message
     * so ElysiumSerializer can avoid 2 instanceof checks + a casting.
     *
     * @return
     */
