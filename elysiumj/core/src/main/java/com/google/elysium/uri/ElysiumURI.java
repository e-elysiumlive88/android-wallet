...
/**
 * <p>Provides a standard implementation of a Elysium URI with support for the
 * following:</p>
 *
 * <ul>
 ...
 *
 * <ul>
 * <li>{@code elysium:<address>}</li>
 * <li>{@code elysium:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
 ...
 * @see <a href="https://en.elysium.it/wiki/BIP_0021">BIP 0021</a>
 */
public class ElysiumURI {
    /**
     * Provides logging for this class
     */
    private static final Logger log = LoggerFactory.getLogger(ElysiumURI.class);
    ...
    /**
     * Constructs a new ElysiumURI from the given string. Can be for any network.
     *
     * @param uri The raw URI data to be parsed (see class comments for accepted formats)
     * @throws ElysiumURIParseException if the URI is not syntactically or semantically valid.
     */
    public ElysiumURI(String uri) {
        this(null, uri);
    }
     ...
     * @throws ElysiumURIParseException If the input fails Elysium URI syntax and semantic checks.
     */
    public ElysiumURI(NetworkParameters params, String input) {
        // Basic validation
        ...
        try {
            ...
            throw new ElysiumURIParseException("Bad URI syntax", e);
        }

        // URI is formed as  elysium:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form elysium://address?....
        // We support both until Ben fixes his code.
        
        // Remove the elysium scheme.
        // (Note: getSchemeSpecificPart() is not used as it unescapes the label and parse then fails.
        // For instance with : elysium:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        // the & (%26) in Tom and Jerry gets interpreted as a separator and the label then gets parsed
        // as 'Tom ' instead of 'Tom & Jerry')
        String schemeSpecificPart;
        if (input.startsWith("elysium://")) {
            schemeSpecificPart = input.substring("elysium://".length());
        } else if (input.startsWith("elysium:")) {
            schemeSpecificPart = input.substring("elysium:".length());
        } else {
            throw new ElysiumURIParseException("Unsupported URI scheme: " + uri.getScheme());
        }

        // Split off the address from the rest of the query parameters.
        String[] addressSplitTokens = schemeSpecificPart.split("\\?");
        if (addressSplitTokens.length == 0 || "".equals(addressSplitTokens[0])) {
            throw new ElysiumURIParseException("Missing address");
        }
        ...
        if (addressSplitTokens.length == 1) {
            ...
            if (addressSplitTokens.length == 2) {
                ...
                throw new ElysiumURIParseException("Too many question marks in URI '" + uri + "'");
            }
        }
    ...
    private void parseParameters(NetworkParameters params, String addressToken, String[] nameValuePairTokens) {
        // Attempt to parse the addressToken as a Elysium address for this network
        try {
            Address address = new Address(params, addressToken);
            putWithValidation(FIELD_ADDRESS, address);
        } catch (final AddressFormatException e) {
            throw new ElysiumURIParseException("Bad address", e);
        }
        ...
        for (String nameValuePairToken : nameValuePairTokens) {
            ...
            if (tokens.length != 2 || "".equals(tokens[0])) {
                throw new ElysiumURIParseException("Malformed Elysium URI - cannot parse name value pair '" +
                        nameValuePairToken + "'");
            }
    ...
    private void putWithValidation(String key, Object value) {
        if (parameterMap.containsKey(key)) {
            throw new ElysiumURIParseException("'" + key + "' is duplicated, URI is invalid");
        } else {
    ...
    /**
     * @return The Elysium Address from the URI
     */
    ...
    public String toString() {
        StringBuilder builder = new StringBuilder("ElysiumURI[");
        boolean first = true;
    ...
    public static String convertToElysiumURI(Address address, BigInteger amount, String label, String message) {
        return convertToElysiumURI(address.toString(), amount, label, message);
    }

    /**
     * Simple Elysium URI builder using known good fields.
     * 
     * @param address The Elysium address
     * @param amount The amount in nanocoins (decimal)
     * @param label A label
     * @param message A message
     * @return A String containing the Elysium URI
     */
    public static String convertToElysiumURI(String address, BigInteger amount, String label, String message) {
        ...
        if (amount != null) {
            ...
            builder.append(Utils.elysiumValueToPlainString(amount));
