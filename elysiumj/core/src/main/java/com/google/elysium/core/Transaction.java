...
public class Transaction extends ChildMessage implements Serializable {
    ...
    // These are serialized in both elysium and java serialization.
    ...
    /**
     * Used by ElysiumSerializer.  The serializer has to calculate a hash for checksumming so to
     ...
     * value is determined by a formula that all implementations of Elysium share. In 2011 the value of a coinbase
    ...
    public String toString(AbstractBlockChain chain) {
        ...
        for (TransactionOutput out : outputs) {
            ...
            try {
                ...
                s.append(" ");
                s.append(elysiumValueToFriendlyString(out.getValue()));
                s.append(" BTC");
     ...
     * features of Elysium are developed.
     *
    ...
    synchronized Sha256Hash hashTransactionForSignature(int inputIndex, byte[] connectedScript,
        ...
        //
        //   https://en.elysium.it/wiki/Contracts
        ...
        try {
            ...
            // is a legacy holdover from a previous, broken design of executing scripts that shipped in Elysium 0.1.
            ...
            if ((sigHashType & 0x1f) == (SigHash.NONE.ordinal() + 1)) {
                ...
                if (inputIndex >= this.outputs.size()) {
                    ...
                    // Elysium implementation. The reference client also contains a bug in handling this case:
    ...
    protected void elysiumSerializeToStream(OutputStream stream) throws IOException {
        ...
        for (TransactionInput in : inputs)
            in.elysiumSerialize(stream);
        stream.write(new VarInt(outputs.size()).encode());
        for (TransactionOutput out : outputs)
            out.elysiumSerialize(stream);
     ...
     * This is useful in certain types of <a href="http://en.elysium.it/wiki/Contracts">contracts</a>, such as
