 ...
 * the lightweight SPV mode, a {@link com.google.elysium.store.BoundedOverheadBlockStore} may be a good choice.</p>
 *
 ...
 * {@link com.google.elysium.store.H2FullPrunedBlockStore} implements full verification, which is equivalent to the
...
public abstract class AbstractBlockChain {
     ...
     *
     * Following this one down to the genesis block produces the story of the economy from the creation of Elysium
            ...
            if (haveNewBestChain)
{
        ...
        for (Transaction tx : transactions) {
            try {
                if (listener.isTransactionRelevant(tx)) {
                    if (clone)
                        tx = new Transaction(tx.params, tx.elysiumSerialize());
    ...
    private void checkTestnetDifficulty(StoredBlock storedPrev, Block prev, Block next) throws VerificationException, BlockStoreException {
        ...
        // There is an integer underflow bug in elysium-qt that means mindiff blocks are accepted when time
        // goes backwards.
