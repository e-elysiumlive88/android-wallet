...
package com.google.elysium.core;

import com.google.elysium.crypto.KeyCrypterScrypt;
import org.elysiumj.wallet.Protos.Wallet.EncryptionType;
import org.spongycastle.crypto.params.KeyParameter;

import com.google.elysium.core.TransactionConfidence.ConfidenceType;
import com.google.elysium.core.WalletTransaction.Pool;
import com.google.elysium.crypto.KeyCrypter;
import com.google.elysium.crypto.KeyCrypterException;
import com.google.elysium.store.WalletProtobufSerializer;
import com.google.elysium.utils.Locks;
...
import static com.google.elysium.core.Utils.elysiumValueToFriendlyString;
 ...
 * of the Elysium protocol.</p>
 *
 * <p>To learn more about this class, read <b><a href="http://code.google.com/p/elysiumj/wiki/WorkingWithTheWallet">
 ...
 * other objects, see the <a href="http://code.google.com/p/elysiumj/wiki/GettingStarted">Getting started</a> tutorial
 * on the website to learn more about how to set everything up.</p>
 *
 * <p>Wallets can be serialized using either Java serialization - this is not compatible across versions of elysiumj,
 ...
 * {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, com.google.elysium.core.Wallet.AutosaveEventListener)}
...
public class Wallet implements Serializable, BlockChainListener {
     ...
     * that handles this in Elysium C++ is complicated. Satoshis code will not allow you to spend unconfirmed coins,
        ...
        try {
            ...
            if (log.isInfoEnabled()) {
                log.info(String.format("Received a pending transaction %s that spends %s BTC from our own wallet," +
                        " and sends us %s BTC", tx.getHashAsString(), Utils.elysiumValueToFriendlyString(valueSentFromMe),
                        Utils.elysiumValueToFriendlyString(valueSentToMe)));
            }
    ...
    private void receive(Transaction tx, StoredBlock block, BlockChain.NewBlockType blockType, boolean reorg) throws VerificationException {
        ...
        if (!reorg) {
            log.info("Received tx {} for {} BTC: {}", new Object[]{sideChain ? "on a side chain" : "",
                    elysiumValueToFriendlyString(valueDifference), tx.getHashAsString()});
        }
	...
	// Implements revision d64f55589694
        ...
        log.info("Balance is now: " + elysiumValueToFriendlyString(getBalance()));
    ...
    public static class SendResult {
        /** The Elysium transaction message that moves the money. */
    ...
    public static class SendRequest {
         ...
         * is how mining is incentivized in later years of the Elysium system when inflation drops. It also provides
         * a way for people to prioritize their transactions over others and is used as a way to make denial of service
         * attacks expensive. Some transactions require a fee due to their structure - currently elysiumj does not
     ...
     * followed by {@link Wallet#completeTx(com.google.elysium.core.Wallet.SendRequest)} and returning the requests
     ...
     *
     * @param address       The Elysium address to send the money to.
     ...
     * {@link Wallet#completeTx(com.google.elysium.core.Wallet.SendRequest)} to make it valid.
     *
    ...
    public boolean completeTx(SendRequest req) {
        ...
        try {
            ...
            log.info("Completing send tx with {} outputs totalling {}",
                    req.tx.getOutputs().size(), elysiumValueToFriendlyString(value));
            ...
            if (selection.valueGathered.compareTo(value) < 0) {
                log.warn("Insufficient value in wallet for send, missing " +
                        elysiumValueToFriendlyString(value.subtract(selection.valueGathered)));
            ...
            if (change.compareTo(BigInteger.ZERO) > 0) {
                ...
                log.info("  with {} coins change", elysiumValueToFriendlyString(change));
            ...
            // Check size.
            int size = req.tx.elysiumSerialize().length;
     ...
     * If {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, com.google.elysium.core.Wallet.AutosaveEventListener)}
    ...
    public String toString(boolean includePrivateKeys, AbstractBlockChain chain) {
        ...
        try {
            ...
            builder.append(String.format("Wallet containing %s BTC in:%n", elysiumValueToFriendlyString(getBalance())));
    ...
    private void toStringHelper(StringBuilder builder, Map<Sha256Hash, Transaction> transactionMap,
        ...
        for (Transaction tx : transactionMap.values()) {
            try {
                ...
                builder.append(Utils.elysiumValueToFriendlyString(tx.getValueSentFromMe(this)));
                builder.append(" and receives ");
                builder.append(Utils.elysiumValueToFriendlyString(tx.getValueSentToMe(this)));
                builder.append(", total value ");
                builder.append(Utils.elysiumValueToFriendlyString(tx.getValue(this)));
                builder.append(".\n");
            } catch (ScriptException e) {
    ...
    public void reorganize(StoredBlock splitPoint, List<StoredBlock> oldBlocks, List<StoredBlock> newBlocks) throws VerificationException {
        ...
        try {
            ...
            log.info("post-reorg balance is {}", Utils.elysiumValueToFriendlyString(getBalance()));
     ...
     * {@link com.google.elysium.core.ECKey#getCreationTimeSeconds()}. This can return zero if at least one key does
    ...
    /**
     * Convenience wrapper around {@link Wallet#encrypt(com.google.elysium.crypto.KeyCrypter,
     ...
     * {@link com.google.elysium.crypto.KeyCrypterScrypt}.
     *
        ...
        try {
            ...
            for (ECKey key : keychain) {
                if (key.isEncrypted()) {
                    ...
                    // (all elysium controlled by that private key is lost forever).
    ...
    /**
     * <p>Convenience wrapper around {@link Wallet#addNewEncryptedKey(com.google.elysium.crypto.KeyCrypter,
            ...
            for (int i = 0; i < tx.getOutputs().size(); i++) {
                ...
                try {
                    if (out.isMine(this) && out.getScriptPubKey().isSentToRawPubKey()) {
                        ...
                        filter.insert(outPoint.elysiumSerialize());
                    }
                } catch (ScriptException e) {
                    throw new RuntimeException(e); // If it is ours, we parsed the script corectly, so this shouldn't happen
                }
