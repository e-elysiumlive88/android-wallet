...
package com.google.elysium.tools;
...
import com.google.elysium.discovery.PeerDiscovery;
import com.google.elysium.store.*;
...
/**
 * A command line tool for manipulating wallets and working with Elysium.<p>
 */
public class WalletTool {
    ...
    public static class Condition {
        ...
        public boolean matchElysiums(BigInteger comparison) {
            try {
    ...
    public static void main(String[] args) throws Exception {
        ...
        switch (netFlag.value(options)) {
            ...
            case TEST: 
                ...
                discovery = new IrcDiscovery("#elysiumTEST3");
                break;
            default:
                throw new RuntimeException("Unreachable.");
        }
    ...
    private static void send(List<String> outputs, BigInteger fee, String lockTimeStr, boolean allowUnconfirmed) {
        try {
            ...
            if (!wallet.completeTx(req)) {
                System.err.println("Insufficient funds: have " + Utils.elysiumValueToFriendlyString(wallet.getBalance()));
                return;
            }
            try {
                if (lockTimeStr != null) {
    ...
    private static void wait(WaitForEnum waitFor) throws BlockStoreException {
        ...
        switch (waitFor) {
            ...
            case BALANCE:
                ...
                if (condition.matchElysiums(wallet.getBalance(Wallet.BalanceType.ESTIMATED))) {
                    latch.countDown();
                    break;
                }
                wallet.addEventListener(new AbstractWalletEventListener() {
                    ...
                    public synchronized void onChange() {
                        ...
                        if (condition.matchElysiums(balance)) {
                            System.out.println(Utils.elysiumValueToFriendlyString(balance));
                            latch.countDown();
                        }
                    }
                });
    ...
    private static void deleteKey() {
        ...
        if (pubkey != null) {
            ...
            try {
                ...
                System.err.println(addr + " does not parse as a Elysium address of the right network parameters.");
                return;
            }            
        }
        if (key == null) {
