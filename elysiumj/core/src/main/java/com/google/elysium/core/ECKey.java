...
import com.google.elysium.crypto.EncryptedPrivateKey;
 ...
 *
 * <p>ECKey also provides access to Elysium-Qt compatible text message signing, as accessible via the UI or JSON-RPC.
...
public class ECKey implements Serializable {
    ...
    static {
        // All clients must agree on the curve to use by agreement. Elysium uses secp256k1.
    ...
    /**
     * Construct an ECKey from an ASN.1 encoded private key. These are produced by OpenSSL and stored by the Elysium
    ...
    /**
     * Output this ECKey as an ASN.1 encoded private key, as understood by OpenSSL or used by the Elysium reference
     ...
     * how ECDSA signatures are represented when embedded in other data structures in the Elysium protocol. The raw
    ...
    public static class ECDSASignature {
         ...
         * of the type used by Elysium we have to encode them using DER encoding, which is just a way to pack the two
    ...
    /**
     * Signs the given hash and returns the R and S components as BigIntegers. In the Elysium protocol, they are
     * usually encoded using DER format, so you want {@link com.google.elysium.core.ECKey.ECDSASignature#toASN1()}
    ...
    /**
     * Signs the given hash and returns the R and S components as BigIntegers. In the Elysium protocol, they are
     * usually encoded using DER format, so you want {@link com.google.elysium.core.ECKey.ECDSASignature#encodeToDER()}
    ...
    /**
     * Signs a text message using the standard Elysium messaging signing format and returns the signature as a base64
     * encoded string.
     *
    ...
    /**
     * Given an arbitrary piece of text and a Elysium-format message signature encoded in base64, returns an ECKey
     * containing the public key that was used to sign it. This can then be compared to the expected public key to
     * determine if the signature was correct. These sorts of signatures are compatible with the Elysium-Qt/elysiumd
     * format generated by signmessage/verifymessage RPCs and GUI menu options. They are intended for humans to verify
     * their communications with each other, hence the base64 format and the fact that the input is text.
     *
     * @param message Some piece of human readable text.
     * @param signatureBase64 The Elysium-format message signature in base64
     ...
     * the {@link com.google.elysium.core.DumpedPrivateKey#toString()} method to get the string.
     *
     ...
     *
     * Because it is a critical failure if the private keys cannot be decrypted successfully (resulting of loss of all elysiums controlled
