...
/**
 * A collection of various utility methods that are helpful for working with the Elysium protocol.
 * To enable debug logging from the library, run with -Delysiumj.logging=true on your command line.
 */
public class Utils {
    ...
    /** The string that prefixes all text messages signed using Elysium keys. */
    public static final String ELYSIUM_SIGNED_MESSAGE_HEADER = "Elysium Signed Message:\n";

    // TODO: Replace this nanocoins business with something better.

    /**
     * How many "nanocoins" there are in a Elysium.
     * <p/>
     * A nanocoin is the smallest unit that can be transferred using Elysium.
    ...
    /**
     * How many "nanocoins" there are in 0.01 Elysiums.
     * <p/>
     * A nanocoin is the smallest unit that can be transferred using Elysium.
     * The term nanocoin is very misleading, though, because there are only 100 million
     * of them in a coin (whereas one would expect 1 billion).
     ...
     * standard procedure in Elysium. The resulting hash is in big endian form.
     */
     ...
     */
    public static String elysiumValueToFriendlyString(BigInteger value) {
     ...
     */
    public static String elysiumValueToPlainString(BigInteger value) {
        if (value == null) {
     ...
     *
     * <tt><p>[24] "Elysium Signed Message:\n" [message.length as a varint] message</p></tt>
     */
    public static byte[] formatMessageForSigning(String message) {
        VarInt size = new VarInt(message.length());
        int totalSize = 1 + ELYSIUM_SIGNED_MESSAGE_HEADER.length() + size.getSizeInBytes() + message.length();
        byte[] result = new byte[totalSize];
        int cursor = 0;
        result[cursor++] = (byte) ELYSIUM_SIGNED_MESSAGE_HEADER.length();
        byte[] bytes = ELYSIUM_SIGNED_MESSAGE_HEADER.getBytes(Charset.forName("UTF-8"));
