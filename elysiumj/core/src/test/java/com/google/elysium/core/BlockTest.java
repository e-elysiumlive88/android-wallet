...
public class BlockTest {
    ...
    public void testHeaderParse() throws Exception {
        ...
        Block reparsed = new Block(params, header.elysiumSerialize());
        assertEquals(reparsed, header);
    }

    @Test
    public void testElysiumSerialization() throws Exception {
        ...
        //
        // NB: This tests the ELYSIUM proprietary serialization protocol. A different test checks Java serialization
        // of transactions.
        Block block = new Block(params, blockBytes);
        assertTrue(Arrays.equals(blockBytes, block.elysiumSerialize()));
    }
    ...
    public void testJavaSerialiazation() throws Exception {
        ...
        // Note that this will actually check the transactions are equal by doing elysium serialization and checking
    ...
    public void testUpdateLength() {
        ...
        assertEquals(block.elysiumSerialize().length, block.length);
        final int origBlockLen = block.length;
        Transaction tx = new Transaction(params);
        // this is broken until the transaction has > 1 input + output (which is required anyway...)
        //assertTrue(tx.length == tx.elysiumSerialize().length && tx.length == 8);
        ...
        assertEquals(tx.elysiumSerialize().length, tx.length);
        ...
        assertEquals(block.length, block.elysiumSerialize().length);
        assertEquals(block.length, origBlockLen + tx.length);
        assertEquals(tx.length, origTxLength - 1);
